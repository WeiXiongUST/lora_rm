...
class ScriptArguments:
    ...
    train_set_path: Optional[str] = field(
        default="weqweasdas/ultrafeedback_binarized_processed",
    ...
    output_path: Optional[str] = field(
        default="./models/llama3_2_1b_rm_lr2e6",
...
def build_dataset(tokenizer, train_path, eval_path):
    ...
    eval_ds = load_dataset(eval_path, split="validation").shuffle(seed=42)
    eval_ds = eval_ds.map(tokenize, num_proc=8)
...
if script_args.use_lora:
    ...
    target_modules = [module.strip() for module in script_args.lora_target_modules.split(",")]
    ...
    print(f"LoRA rank: {script_args.lora_r}, alpha: {script_args.lora_alpha}, dropout: {script_args.lora_dropout}")
else:
    print("Training without LoRA - using full model fine-tuning")
...
if script_args.use_lora:
    ...
    print(f"LoRA adapters saved to {output_name}/last_checkpoint")
else:
    # For full model fine-tuning
    trainer.save_model(output_name + "/last_checkpoint")
    print(f"Full model saved to {output_name}/last_checkpoint")
